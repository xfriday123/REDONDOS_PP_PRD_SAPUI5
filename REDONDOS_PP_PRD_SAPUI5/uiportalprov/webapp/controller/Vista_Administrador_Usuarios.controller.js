sap.ui.define([
    ...
    "sap/m/MessageBox",    
], function (Controller, Export, ExportTypeCSV, Spreadsheet, MessageToast, BusyIndicator,MessageBox) {
    "use strict";
    return Controller.extend("nsrdos.uiportalprov.controller.Vista_Administrador_Usuarios", {
        varTableURL: "",
        varTableDocument: "",
        varTableT_CEN: "",
        varTableT_CON: "",
        ...
        obtenerListAnuncios: function () {
            var oModel = this.getView().getModel("myParam");
            //obtener imagen de banner 
            this.oGlobalIteracion0 = "0";
            $.ajax(this.varTableURL + "/T_ANUNCIOS?$format=json&$skiptoken=" + this.oGlobalIteracion0, {
                headers: { 'X-CSRF-Token': this.token },
                type: "GET",
                contentType: "application/json",
                dataType: "json",
                success: function (res) { 
                    console.log("obtenerListAnuncios", res.value);
                    oModel.setProperty("/listAnunciosAdmin",res.value); 
                }.bind(this),
        ...
        funInsertarRegistro: function () {
            ...
            $.ajax(this.varTableURL + "/" + this.varTableT_SERVICIOS, {
                headers: oHeaders,
                data: oVectorJSON,
                type: "POST",
                contentType: "application/json",
        ...
        MessageBoxPressQuery: function (typeMsm,titleMsm) {
            return new Promise((resolve, reject) => {  
                MessageBox[typeMsm](titleMsm, {
                    ...
                    onClose: function (sAction) {
                        let res = false
                        if(sAction === MessageBox.Action.OK){ res = true }  
                        resolve(res); 
                    }
                });
            }); 
        ...
        f_imgPreSubida: async function () { 
            var oModel = this.getView().getModel("myParam");
            const oFileUploader = this.getView().byId("txt_Imagenes");
            var imgFile = oFileUploader.oFileUpload.files[0]; 
            var binary; 
            if (imgFile ) { 
                var reader = new FileReader();
                reader.onload = function (readerEvt) {
                    var binaryString = readerEvt.target.result;
                    binary = btoa(binaryString); 
                    console.log("binary",binary);
                    this.getView().byId("idImagenPrevio").setSrc("data:image/jpg;base64," + binary);
                    this.getView().byId("btn_upImagen").setEnabled(true);
                    let v_estado = this.getView().byId("chk_estadoImg").getSelected();
                    let v_imgUrl = this.getView().byId("txt_urlImg").getValue();
                    if(v_estado){v_estado = 'X'} else{v_estado = ''};
                    var fileImg = { 
                        FILE_IMG_NOMBRE : imgFile.name, 
                        FILE_IMG_TIPO : imgFile.type,
                        FILE_IMG_BINARIO: binary,
                        ESTADO: v_estado, 
                        FILE_IMG_URL: v_imgUrl, 
                    } 
                    console.log("fileImg",fileImg) 
                    this.getView().byId("chk_estadoImg").setEnabled(true);
                    this.getView().byId("txt_urlImg").setValue("");
                    oModel.setProperty("/imgPreSubida",fileImg)
                }.bind(this); 
                reader.readAsBinaryString(imgFile);
            }else{
                MessageToast.show("Seleccione un imagen.");
                this.getView().byId("idImagenPrevio").setSrc('');
                this.getView().byId("btn_upImagen").setEnabled(false);
            }
        },
        rowSelectionChangeAnuncio: function (evt) {
            var context = evt.getParameters().rowBindingContext; 
            var objetoSelect = context.getObject();
            console.log(objetoSelect);
            if (!objetoSelect.FILE_IMG_BINARIO) { MessageToast.show("El item seleccionado no tiene una imagen."); return; }
            this.getView().byId("idImagenPrevio").setSrc("data:image/jpg;base64," + objetoSelect.FILE_IMG_BINARIO);
        },
        f_changeAnuncioPublicar:  function (oEvent) {
            var oItem = oEvent.getSource();
            var oContext = oItem.getBindingContext("myParam");
            var objetoSelect = oContext.getObject();
            if(objetoSelect.ESTADO === 'x' || objetoSelect.ESTADO === 'X'){ MessageBox.information("El item ya esta publicado"); return;}
            this.f_changeAnuncioEstado(objetoSelect,'X');
        },
        f_changeAnuncioNoPublicar:  function (oEvent) {
            var oItem = oEvent.getSource();
            var oContext = oItem.getBindingContext("myParam");
            var objetoSelect = oContext.getObject();
            if(objetoSelect.ESTADO === ''){ MessageBox.information("El item ya esta en estado No publicado"); return;}
            this.f_changeAnuncioEstado(objetoSelect,'');
        },
        f_changeAnuncioEstado: async function (objetoSelect, estado) {
            let ok = await this.MessageBoxPressQuery("information","¿Desea cambiar su estado de esta imagen?")
            if(ok){
                let llave = {ESTADO: estado}
                this.oGlobalIteracion0 = "0";
                // $.ajax(this.varTableURL + "/T_ANUNCIOS&$filter=FILE_IMG_NOMBRE eq '"+ objetoSelect.FILE_IMG_NOMBRE +"'", {
                $.ajax(this.varTableURL + "/T_ANUNCIOS(FILE_IMG_NOMBRE='"+ objetoSelect.FILE_IMG_NOMBRE +"')", {
                    // $.ajax(this.varTableURL + "/" + this.varTableT_DETRACCION + "(NUMTP='" + varNumReg + "')", {
                    // $.ajax(this.varTableURL + "/" + this.varTableT_DETRACCION + "(NUMTP='" + varNumReg + "')", {
                    headers: { 'X-CSRF-Token': this.token },
                    ...
                    dataType: "json",
                    success: function (data) {
                        MessageBox.success("Anuncio actualizado"); 
                        console.log("anuncio actualizado"); 
                        this.obtenerListAnuncios();
                    }.bind(this),
                    error: function (error) { MessageBox.information("Anuncio no actualizado"); console.log("error actualizar registro de anuncio",error); }.bind(this)
                });
            }else{ MessageToast.show("Solicitud cancelada.")}
        },
        f_changePublicar: function (oEvent) {
            var oModel = this.getView().getModel("myParam");
            let imgUp = oModel.getProperty("/imgPreSubida");
            let v_estado = this.getView().byId("chk_estadoImg").getSelected();
            if(v_estado){v_estado = 'X'} else{v_estado = ''};//
            let imgUpEstado = {...imgUp,ESTADO:v_estado}
            oModel.setProperty("/imgPreSubida",imgUpEstado);
        },
        f_upImagen: async function () {
            var oModel = this.getView().getModel("myParam");                
            let imgUp = oModel.getProperty("/imgPreSubida");        
            let v_imgUrl = this.getView().byId("txt_urlImg").getValue();
            let imgUpEstado = {...imgUp,FILE_IMG_URL:v_imgUrl}
            oModel.setProperty("/imgPreSubida",imgUpEstado);
            let ok = await this.MessageBoxPressQuery("information","Seguro de subir esta imagen?")
            if(ok){
                var oModel = this.getView().getModel("myParam");                
                let imgUp = oModel.getProperty("/imgPreSubida");
                //Subiendo la imagen
                var oHeaders = { 'X-CSRF-Token': this.token };
                this.oGlobalIteracion0 = "0";
                $.ajax(this.varTableURL + "/T_ANUNCIOS", { 
                    headers: oHeaders,
                    data: JSON.stringify(imgUp),
                    type: "POST",
                    contentType: "application/json",
                    dataType: "json",
                    success: function (data) { 
                        MessageBox.success("Anuncio subido");
                        this.obtenerListAnuncios();
                        // Limpiar form 
                        this.getView().byId("idImagenPrevio").setSrc("");
                        this.getView().byId("txt_Imagenes").setValue("");
                        this.getView().byId("txt_urlImg").setValue("");
                        this.getView().byId("chk_estadoImg").setSelected(false);
                        this.getView().byId("chk_estadoImg").setEnabled(false);
                        this.getView().byId("btn_upImagen").setEnabled(false);
                    }.bind(this),
                    error: function (error) { 
                        MessageBox.information("Anuncio no subido, segurece que no exista un archivo con el mismo nombre.");
                        console.log("error actualizar imagene publicada",error);
                    }.bind(this)
                });
 
            }else{ MessageToast.show("Solicitud cancelada.")}
        },
        f_deleteImagen: async function (oEvent) {
            let ok = await this.MessageBoxPressQuery("information","Seguro de eliminar esta imagen?")
            if(ok){
                var oModel = this.getView().getModel("myParam"); 
                var oItem = oEvent.getSource();
                var oContext = oItem.getBindingContext("myParam");
                ...
                this.oGlobalIteracion0 = "0";
                var oHeaders = { 'X-CSRF-Token': this.token };
                // $.ajax(this.varTableURL + "/T_ANUNCIOS?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=FILE_IMG_NOMBRE eq '"+ objetoSelect.FILE_IMG_NOMBRE +"'", {
                $.ajax(this.varTableURL + "/T_ANUNCIOS(FILE_IMG_NOMBRE='"+ objetoSelect.FILE_IMG_NOMBRE +"')", {
                    headers: oHeaders, 
                    type: "DELETE",
                    contentType: "application/json",
                    dataType: "json",
                    success: function (data) { 
                        MessageBox.success("Anuncio eliminado.");
                        this.obtenerListAnuncios();
                    }.bind(this),
                    error: function (error) { 
                        MessageBox.information("Anuncio no eliminado.")
                        console.log("error actualizar imagene publicada",error);
                    }.bind(this)
                });
        ...
        formatXtoBooleanAnuncio: function (value) {
            let res = false;
            if(value === 'X' || value === 'x'){res = true}
            return res;
        },         
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);
                            var oHeaders = { 'X-CSRF-Token': this.token };
                            $.ajax(this.varTableURL + "/" + this.varTableT_DETRACCION, {
                                headers: oHeaders,
                                data: oVectorJSON,
                                type: "POST",
                                contentType: "application/json",
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);	
                            var oHeaders = { 'X-CSRF-Token': this.token };
                            $.ajax(this.varTableURL + "/" + this.varTableT_MAT_DETRACCION, {
                                headers: oHeaders,
                                data: oVectorJSON,
                                type: "POST",
                                contentType: "application/json",
            ...
            var dialog2 = new sap.m.Dialog({
                ...
                content: [
                    ...
                    new sap.m.Toolbar({
                        ...
                        width: "100%",
                        content: [new sap.m.Label({
                            text: "Sujeto a valor referencial : ",
                            textAlign: "Right",
                            width: "30%"
                        }),
                            checkBox03,
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);	
                            var oHeaders = { 'X-CSRF-Token': this.token };
                            $.ajax(this.varTableURL + "/" + this.varTableT_SERV_DETRACCION, {	
                                headers: oHeaders,
                                data: oVectorJSON,
                                type: "POST",
                                contentType: "application/json",
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            console.log(this.token);
                            var oHeaders = { 'X-CSRF-Token': this.token }
                            $.ajax(this.varTableURL + "/" + this.varTableT_MAT_DETRACCION + "(DE_NUM_MATERIAL='" + varMatReg + "')", {
                                type: 'PATCH',
                                data: oVectorJSON,		
                                headers: oHeaders,						
                                contentType: "application/json",
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);
                            var oHeaders = { 'X-CSRF-Token': this.token }
                            $.ajax(this.varTableURL + "/" + this.varTableT_SERV_DETRACCION + "(DE_NUM_SERVICIO='" + varMatReg + "')", {
                                type: 'PATCH',
                                data: oVectorJSON,		
                                headers: oHeaders,						
                                contentType: "application/json",
            ...
            var dialog2 = new sap.m.Dialog({
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        var oHeaders = { 'X-CSRF-Token': this.token }
                        $.ajax(this.varTableURL + "/" + this.varTableT_MAT_DETRACCION + "(DE_NUM_MATERIAL='" + varNumReg + "')", {
                            type: 'DELETE',						
                            headers: oHeaders,
                            contentType: "application/json",
                            dataType: "json",
            ...
            var dialog2 = new sap.m.Dialog({
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        var oHeaders = { 'X-CSRF-Token': this.token }
                        $.ajax(this.varTableURL + "/" + this.varTableT_SERV_DETRACCION + "(DE_NUM_SERVICIO='" + varNumReg + "')", {
                            type: 'DELETE',						
                            headers: oHeaders,
                            contentType: "application/json",
                            dataType: "json",
        ...
        llenarTablas: function (tabla, json) {
            ...
            while (this.oGlobalStop10 === "S") {
                $.ajax({
                    type: "GET",
                    async: false,
                    url: this.varTableURL + "/" + tabla + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "",
        ...
        formatNombreServicio: function (servicio) {
            ...
            } else if (servicio === "Cantidad_habiles_para_facturar") { //MM-20240207-MEJORA
                return "Cantidad hábiles para facturar";
            } else {
                return servicio;
            }
        },
        ...
        pressActualizarServicio: function (evt) {
            ...
            if (objeto.SERVICIO === "Validacion_CPE_SUNAT") {
                    ...
                    var dialogA = new sap.m.Dialog({
                        ...
                        beginButton: new sap.m.Button({
                            ...
                            press: function () {
                                ...
                                var oHeaders = { 'X-CSRF-Token': this.token }
                                $.ajax(this.varTableURL + "/" + this.varTableT_SERVICIOS + "(SERVICIO='" + objeto.SERVICIO + "')", {
                                    type: 'PATCH',
                                    data: oVectorJSON,		
                                    headers: oHeaders,						
                                    contentType: "application/json",
                ...
                dialogA.open();
            } else if (objeto.SERVICIO === "Cantidad_habiles_para_facturar") { //MM-20240207-MEJORA
                var dialogA = new sap.m.Dialog({
                    title: "Mensaje de confirmación",
                    type: "Message",
                    state: "Success",
                    content: [
                        new sap.m.Toolbar({
                            ...
                            width: "100%",
                            content: [new sap.m.Label({
                                text: "Cantidad de dias para facturar: ",
                                textAlign: "Right",
                                width: "55%"
                            }),
                            new sap.m.Input({
                    ...
                    new sap.m.Toolbar({
                        ...
                        new sap.m.Button({
                            ...
                            press: function () {
                                ...
                                var oVectorJSON = JSON.stringify(llave);
                                var oHeaders = { 'X-CSRF-Token': this.token }
                                $.ajax(this.varTableURL + "/" + this.varTableT_USER + "(US_USUARIO='" + objeto.US_USUARIO + "')", {
                                    type: 'PATCH',
                                    data: oVectorJSON,		
                                    headers: oHeaders,						
                                    contentType: "application/json",
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);	
                            var oHeaders = { 'X-CSRF-Token': this.token };
                            $.ajax(this.varTableURL + "/" + this.varTableT_CEN, {	
                                headers: oHeaders,
                                data: oVectorJSON,
                                type: "POST",
                                contentType: "application/json",
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function () {
                        ...
                        if (canContinue) {
                            ...
                            var oVectorJSON = JSON.stringify(llave);
                            var oHeaders = { 'X-CSRF-Token': this.token }
                            $.ajax(this.varTableURL + "/" + this.varTableT_CEN + "(RUC_EM='" + varRucReg + "')", {
                                type: 'PATCH',
                                data: oVectorJSON,		
                                headers: oHeaders,						
                                contentType: "application/json",
        ...
        handleLiveChange3: function (tabla, oEvent) {
			var newValue = oEvent.getParameter("value");
			var filter;
			var filters1 = [];
			var filtersT = [];
        ...
        formatoEstadoTextBoton: function (est) {
            if (est !== "" && est !== null && est !== undefined) {
                if (est === "Validacion_CPE_SUNAT") {
                    ...
                    return "Editar";
                } else if (est === "Cantidad_habiles_para_facturar") { //MM-20240207-MEJORA
                    return "Editar";
                }
            }
        },
        ...
        btnDescargarFacturasTotal: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_FAC + "?$format=json",
                success: function (response) {
                    var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
                    var tamTabla = oModelJSON.getData().length;
                    var vector = [];
