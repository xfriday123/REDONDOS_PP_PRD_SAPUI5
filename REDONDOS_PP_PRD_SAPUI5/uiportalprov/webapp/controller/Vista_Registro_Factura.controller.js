sap.ui.define([
	...
	return Controller.extend("nsrdos.uiportalprov.controller.Vista_Registro_Factura", {
        ...
        metFadeBoxIni: function () {
            this.byId("idHTMLContent").setContent("<div id='splash-screen'><div class='splash-screen-text'>Cargando..</div><div class='splash-screen-circle-outer'></div><div class='splash-screen-circle-inner'></div></div>");
        },

        metFadeBoxFin: function () {
		...
		onAfterRendering: function () {
            ...
            this.oBDURl2 = oModel.getProperty("/listurlBD/bdClientes2");
			// Tablas
            ...
            oModel.setProperty("/usuarioLogin", oModel.getProperty("/listUsuario"));
            ...
            while (this.oGlobalStop10 === "S") {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (this.oEntrar === "S") {
                            try {
                                var oModelJSON5 = oModel9.getProperty("/oListaVectorCabeceraDetalle");
                                var oModelJSON = new sap.ui.model.json.JSONModel(oModelJSON5);
            ...
            var comboDetra2 = this.getView().byId("idCodigoDetra2");//I@LL-29/04/2022-Ticket-2022-0602
			comboDetra2.getBinding("items").refresh(true);
			var firstItem2 = comboDetra2.getItems()[0];
			comboDetra2.setSelectedItem(firstItem2, true);
        ...
        metObtenerTipoDeCambio: function () { //20220520
            ...
            var oI_BUKRS = "1000";
            //var oI_FECHA = "19102020";
            var oI_FECHA = dia + mes + anio;
            console.log(oI_FECHA);
            var oI_MONEDA = "USD";

            $.ajax({
                type: "GET",
                url: "/s4h_tc/" + oI_BUKRS + "/" + oI_FECHA + "/" + oI_MONEDA,
                success: function (response) {
                    console.log(response);
                    try {
                        var oModelJSON = response.ITAB[0];
                        console.log(oModelJSON);

                        oModel.setProperty("/oGlobalTipoDeCambio", oModelJSON.TIPO_CAMBIO);
                        console.log(oModel.getProperty("/oGlobalTipoDeCambio"));
                    } catch (err) {
                        console.log(err);
                    }
                }.bind(this),
            ...
            oVector = JSON.stringify(oVector);

            var oHeaders = { 'X-CSRF-Token': this.token };
            $.ajax(this.oBDURl + "obtenerTC", {
                //$.ajax("http://localhost:4004/browse/inListClientes", {
                type: "POST",
                async: false,
                data: oVector,
                headers: oHeaders,
                ...
                success: function (data) {
                    console.log(data);
                    var oXml = data.myResponse;
                    var domParser = new DOMParser();
                    var xmlDocument = domParser.parseFromString(oXml, "text/xml");

                    var oJSON = this.convertirXmlEnObjeto(xmlDocument);
                    oModel.setProperty("/listTblOdataeRepPedido", oJSON);
                    console.log(oModel.getProperty("/listTblOdataeRepPedido"));
                    var oDataRepPedidoCab = oModel.getProperty("/listTblOdataeRepPedido/SOAP:Body/ns1:MT_Extraer_Pedido_Response/ET_CABPEDIDO/item");
                    console.log(oDataRepPedidoCab);
                }.bind(this),
		...
		_onRouteMatched: function () {
            ...
            /*$.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_OC_DET + "?$format=json&$filter=US_RUC eq '" + varUsuario + "' and EM_RUC eq '" + this.varRucDeLaEmpresa + "'",
                success: function (response) {
		...
		updateServicioDiaHabil: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_SERVICIOS + "?$format=json&$filter=SERVICIO eq '" + "Validacion_dias_habiles" + "'",
                success: function (response) {
					var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
		...
		updateServicioDiaCalendario: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_SERVICIOS + "?$format=json&$filter=SERVICIO eq '" + "Validacion_dias_calendario" + "'",
                success: function (response) {
					var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
					var lenghtV = oModelJSON.getData().length;
					if (lenghtV !== 0) {
		...
		updateServicioTolerancia: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_SERVICIOS + "?$format=json&$filter=SERVICIO eq '" + "Validacion_porcentaje_tolerancia" + "'",
                success: function (response) {
					var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
					var lenghtV = oModelJSON.getData().length;
					if (lenghtV !== 0) {
        ...
        funObtenerIndIndicador: function (varMaterial) {
            ...
            if (comboKey === "S") {
                ...
                var oDataIndiAdmi = oModel.getProperty("/listServDetraccion");
                for (var i = 0; i < oDataIndiAdmi.length; i++) {
                    if (oDataIndiAdmi[i].DE_NUM_SERVICIO === oMaterial) {
                        oRespuesta = oDataIndiAdmi[i].NUMTP;
                    }
                }
            }
        ...
        funObtenerIndPorcentage: function (varMaterial) {
        ...
        funObtenerIndMonto: function (varMaterial) {
        ...
        funObtenerMonto: function (oCodDetr) { //20220511

            var oThis = this;
			var oModel = oThis.getView().getModel("myParam");

            var listCodigoDetr = oModel.getProperty("/codigoDetr");
            var oRespuesta = "";

            for (var i = 0; i < listCodigoDetr.length; i++) {
                if (oCodDetr === listCodigoDetr[i].codigo) {
                    oRespuesta = listCodigoDetr[i].monto
                }
            }
		...
		btnBuscarItem2: function (oEvent) {
            ...
            var oModelListaValesIngreso = oModel.getProperty("/listaValesIngreso");
            var oVector_Ind = []
            var oLlave_Ind = {};
            if (comboKey === "S") {
                for (var i = 0; i < oModelListaValesIngreso.length; i++) {
                    oLlave_Ind = {};
                    oLlave_Ind = oModelListaValesIngreso[i];
                    oLlave_Ind.IND_INDICADOR = this.funObtenerIndIndicador(parseInt(oModelListaValesIngreso[i].DE_NUM_MATERIAL, 10).toString());
                    oLlave_Ind.IND_PORCENTAJE = this.funObtenerIndPorcentage(parseInt(oModelListaValesIngreso[i].DE_NUM_MATERIAL, 10).toString());
                    oLlave_Ind.IND_MONTO = this.funObtenerIndMonto(parseInt(oModelListaValesIngreso[i].DE_NUM_MATERIAL, 10).toString());
                    oVector_Ind.push(oLlave_Ind);
                }
            } else if (comboKey === "H") {
                for (var i = 0; i < oModelListaValesIngreso.length; i++) {
                    oLlave_Ind = {};
                    oLlave_Ind = oModelListaValesIngreso[i];
                    oLlave_Ind.IND_INDICADOR = this.funObtenerIndIndicador(parseInt(oModelListaValesIngreso[i].DE_NUM_SERVICIO, 10).toString());
                    oLlave_Ind.IND_PORCENTAJE = this.funObtenerIndPorcentage(parseInt(oModelListaValesIngreso[i].DE_NUM_SERVICIO, 10).toString());
                    oLlave_Ind.IND_MONTO = this.funObtenerIndMonto(parseInt(oModelListaValesIngreso[i].DE_NUM_SERVICIO, 10).toString());
                    oVector_Ind.push(oLlave_Ind);
                }
            }
            oModel.setProperty("/listaValesIngreso", oVector_Ind);
            //End I@MM-01/03/2022-Ticket-2022-581
					...
					press: function () {
						...
						if (varSelecTipoCarga === "H") {
							...
							for (var i = 0; i < vector.length; i++) {
								for (var j = 0; j < oVectorlistServValReferencial.length; j++) {
									if (parseInt(vector[i].DE_NUM_SERVICIO, 10).toString() === oVectorlistServValReferencial[j].DE_NUM_SERVICIO &&
						...
						if (this.oGlobalCodTipoDetraccion === "" || this.oGlobalCodTipoDetraccion === undefined || this.oGlobalCodTipoDetraccion ===
								...
								for (var i = 0; i < oVectorCodMatServ.length; i++) {
									for (var j = 0; j < oTblMatDetraccion.length; j++) {
										if (oVectorCodMatServ[i].DE_NUM_MATERIAL === oTblMatDetraccion[j].DE_NUM_MATERIAL) {
                                            ...
                                            oLlaveDet.FLAG2 = oTblMatDetraccion[j].FLAG2; //20220310
								...
								for (var i = 0; i < oVectorCodMatServ.length; i++) {
									for (var j = 0; j < oTblServDetraccion.length; j++) {
										if (oVectorCodMatServ[i].DE_NUM_SERVICIO === oTblServDetraccion[j].DE_NUM_SERVICIO) {
                                            ...
                                            oLlaveDet.FLAG2 = oTblServDetraccion[j].FLAG2; //20220310
											oVectorDet.push(oLlaveDet);
										}
									}
								}
							...
							for (var i = 0; i < oVectorDet.length; i++) {
								if (oVectorDet[i].NUMTP !== "N.A." || oVectorDet[i].NUMTP !== "NA") {
									oContDetNA++;
								}
							}
							...
							if (oContDetNA === 0) {
                                ...
                                console.log(this.oGlobalCodMontoTotal); //20220511
                                this.oGlobalCodMontoDetraccion = this.funObtenerMonto(oCodFinalDetraccion); //20220511
                                console.log(this.oGlobalCodMontoDetraccion); //20220511
                                //this.getView().byId("idCodigoDetra2").setValue(oCodFinalDetraccion + " - " + oDesFinalDetraccion);
                                var varTipoDeCambio1 = 1;
                                if (cabecera[8].value.trim() === "USD") {
                                    varTipoDeCambio1 = oModel.getProperty("/oGlobalTipoDeCambio");
                                } else {
                                    varTipoDeCambio1 = 1;
                                }
                                console.log(varTipoDeCambio1*parseFloat(this.oGlobalCodMontoTotal, 10) + " > " + parseFloat(this.oGlobalCodMontoDetraccion, 10)) //20220511
                                var oResultadoFloat0 = parseFloat(this.oGlobalCodMontoTotal, 10);
                                var oResultadoFloat1 = varTipoDeCambio1*oResultadoFloat0;
                                console.log(parseFloat(this.oGlobalCodMontoTotal));
                                console.log(parseFloat(this.oGlobalCodMontoTotal, 10));
                                console.log(oResultadoFloat1);
                                if (varTipoDeCambio1*parseFloat(this.oGlobalCodMontoTotal, 10) >= parseFloat(this.oGlobalCodMontoDetraccion, 10)) { //20220511
                                    var varValorIGVvALIDAR1 = parseInt(this.getView().byId("idTotalIGV").getValue(), 10);
                                    console.log(varValorIGVvALIDAR1);
                                    if (varValorIGVvALIDAR1 !== 0) {
                                ...
                                //Begin I@MM-01/03/2022-Ticket-2022-581
                                console.log(this.oGlobalSignSUNAT);
                                console.log(this.oGlobalCodMontoTotal);
                                console.log(this.oGlobalCodMontoDetraccion);
                                if (this.oGlobalSignSUNAT.trim() === "") {
                                    if (cabecera[8].value.trim() === "USD") {
                                        varTipoDeCambio1 = oModel.getProperty("/oGlobalTipoDeCambio");
                                    } else {
                                        varTipoDeCambio1 = 1;
                                    }
                                    if (varTipoDeCambio1*parseFloat(this.oGlobalCodMontoTotal, 10) >= parseFloat(this.oGlobalCodMontoDetraccion, 10)) {
                                        if (oCodFinalDetraccion === this.oGlobalCodTipoDetraccion) {
                                            this.getView().byId("idValidacionDeta").setText("Detracción establecida.");
                                            this.getView().byId("idValidacionDeta").setType("Success");
                                        } else {
                                            this.getView().byId("idValidacionDeta").setText("Detracción establecida."); //20230608
                                            this.getView().byId("idValidacionDeta").setType("Success");
                                        }
                                    } else {
                                        this.getView().byId("idValidacionDeta").setText("Detracción establecida.");
								...
								for (var i = 0; i < oVectorDet.length; i++) {
									for (var j = 0; j < oTblDetraccion.length; j++) {
										if (oVectorDet[i].NUMTP === oTblDetraccion[j].NUMTP && oVectorDet[i].NUMTP !== "N.A."/* && oVectorDet[i].FLAG2 !== ""*/) { //20220310
                                            console.log(oVectorDet[i].FLAG2);
											oLlaveDetPri = {};
								...
								//this.validarSunat(); //I@MM-01/03/2022-Ticket-2022-581
								this.verificarAsignaciónPosFactura();
							}
						} else {
							var oVectorDet = [];
		...
		sortJSON: function (data, key, orden) { // MAURO 20211014
			return data.sort(function (a, b) {
				var x = parseInt(a[key], 10), //20220310
					y = parseInt(b[key], 10); //20220310
                console.log(x);
                console.log(y);
			...
			var dialogMensaje = new sap.m.Dialog({
				...
				beginButton: new sap.m.Button({
					press: function () {
						...
						console.log("OK");
						this.getView().byId("idValorReferencial").setValue(""); //I@MM-21/12/2021-Ticket-2021-999
						if (llave === "S") {
							this.getView().byId("idLabelValorReferencial").setVisible(false); //I@MM-21/12/2021-Ticket-2021-999
							this.getView().byId("idValorReferencial").setVisible(false); //I@MM-21/12/2021-Ticket-2021-999
							this.getView().byId("idUdpLabelValorReferencial").setVisible(false); //I@MM-21/12/2021-Ticket-2021-999
		...
		oDialogOrdenDeCompra: function (varPosicionFactura, varCodigoFactura, varValortotalNetoFactura, varDescripcionFactura,
					...
					/*oModelxd.read("/" + this.varTableT_OC_DET + "?$format=json", {
                        ...
                        while (this.oGlobalStop10 === "S") {
                            $.ajax({
                                type: "GET",
                                async: false,
                                url: this.varTableURL + "/" + this.varTableT_OC_DET + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=DE_NUMERO_ORDEN eq '" + vlistOrdenCompraNum + "' and DE_FLAC ne '" + "x" + "' and DE_FLAC ne '" + "X" + "'",
                                success: function (response) {
			...
			var oDialogSelectOrdenCompra = new sap.m.Dialog("idDialogOrdenCompra", {
				...
				afterOpen: function () {
                    ...
                    $.ajax({
                        type: "GET",
                        url: this.varTableURL + "/" + this.varTableT_OC + "?$format=json",
                        success: function (response) {
                            var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
							var realizar = false;
							var itemsOrdenCompra = oModel.getProperty(pathFacturaItem + "/clistItemsOrdenCompra");
		...
		fun_sumar_dias: function (fecha, dias) { //MM-20240207-MEJORA
			fecha.setDate(fecha.getDate() + dias);
			return fecha;
		},

		met_obtener_cant_dias_hab: function () { //MM-20240207-MEJORA
			...
			return new Promise((resolve, reject) => {
				...
				$.ajax({
					type: "GET",
					url: this.varTableURL + "/" + this.varTableT_SERVICIOS + "?$format=json",
					success: function (response) {
						console.log("OK - Cantidad_habiles_para_facturar");
						var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
						var tamTabla = oModelJSON.getData().length;
						for (var i = 0; i < tamTabla; i++) {
							if (oModelJSON.getData()[i].SERVICIO === "Cantidad_habiles_para_facturar") {
								var valorServicio = parseInt(oModelJSON.getData()[i].CAMPO1.toString());
								/* var d = new Date();
								d.setDate(d.getDate() - valorServicio); */
								oModel.setProperty("/oGlobalCantDiasHab", valorServicio);
								resolve(valorServicio);
							}
						}
					}.bind(this),
					error: function (oError) {
						console.log("ERROR - Cantidad_habiles_para_facturar");
						reject("");
					}.bind(this)
				});
			});
		...
		met_obtener_devoluciones: function () { //MM-20240207-MEJORA
			...
			return new Promise((resolve, reject) => {
				...
				this.oGlobalStop10 = "S";
				this.oGlobalIteracion0 = "0";
				oModel9.setProperty("/oListaVectorCabeceraDetalle2", []);
				this.oEntrar = "N";
				var varUsuario = oModel9.getProperty("/usuarioLogin");

				while (this.oGlobalStop10 === "S") {
					$.ajax({
						type: "GET",
						async: false,
						url: this.varTableURL + "/" + this.varTableT_OC_DET + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=US_RUC eq '" + varUsuario + "' and DE_FLAC ne '" + "x" + "' and DE_FLAC ne '" + "X" + "' and DE_TIPO eq '" + "D" + "'",
						success: function (response) {
							...
							if (oDataHana.length !== 0) {
								...
								var oMatriz = oModel9.getProperty("/oListaVectorCabeceraDetalle2");
								var oVector = {};
								if (oDataHana.length !== 0) {
									for (var i = 0; i < oDataHana.length; i++) {
										oVector = {};
									...
									oModel9.setProperty("/oListaVectorCabeceraDetalle2", oMatriz);
								} else {
									oModel9.setProperty("/oListaVectorCabeceraDetalle2", []);
								}
								console.log(oModel9.getProperty("/oListaVectorCabeceraDetalle2"));
							...
							if (this.oEntrar === "S") {
								var oVectorCabeceraDetalle = oModel9.getProperty("/oListaVectorCabeceraDetalle2");
								try {
									var oModelJSON = new sap.ui.model.json.JSONModel(oVectorCabeceraDetalle);
									var tamTabla = oModelJSON.getData().length;
									var vector = [];
									...
									oModel9.setProperty("/listaValesIngreso2", vector);
									resolve(vector);
								} catch (err) {
									oModel9.setProperty("/listaValesIngreso2", []);
									reject([]);
								}
							}
						}.bind(this),
		...
		btnAnadirCargarXMLTabla: async function (e) { //MM-20240207-MEJORA

			var o_dias = await this.met_obtener_cant_dias_hab(); //MM-20240207-MEJORA
			var o_vector_devoluciones = await this.met_obtener_devoluciones(); //MM-20240207-MEJORA
			console.log(o_dias);
			console.log(o_vector_devoluciones);

			var o_fec_1_h = new Date();
			o_fec_1_h.setHours(0, 0, 0, 0);

			if (o_vector_devoluciones.length !== 0) {
				var o_ordenar = o_vector_devoluciones.sort((a, b) => (a.DE_FEC_REGISTRO < b.DE_FEC_REGISTRO) ? 1 : ((b.DE_FEC_REGISTRO < a.DE_FEC_REGISTRO) ? -1 : 0));
				var o_fec_1 = o_ordenar[0].DE_FEC_REGISTRO;

				var o_fec_1_i = new Date(o_fec_1 + " 00:00:00");
				var o_fec_1_f = this.fun_sumar_dias(o_fec_1_i, o_dias);
				console.log(o_fec_1_i);
				console.log(o_fec_1_f);
				console.log(o_fec_1_h);

				if (o_fec_1_f < o_fec_1_h) {
					var dialog = new sap.m.Dialog({
						title: "Alerta",
						type: "Message",
						state: "Warning",
						content: new sap.m.Text({
							text: "Tiene notas de creditos pendientes, por tanto no podrá generar su factura."
		...
		validarFacturaFechaEmision: function (varFechaEmision) {
			...
			var anio_2 = varFechaEmision.substring(0, 4);
			...
			console.log(varMesProximo1);
			if (varMesProximo1 === 13) {
				varMesProximo1 = "1";
				//dateFormatted1.setFullYear(parseInt(dateFormatted1.getFullYear(), 10) + 1); 20220104
				anio_2 = parseInt(anio_2) + 1;
				anio_2 = anio_2.toString();
			}
			console.log(varMesProximo1);
			console.log(dateFormatted1);
			var varFechaHabile1 = anio_2 + "-" + varMesProximo1 + "-" + varMatrizDiasHabiles[0].CAMPO1;
			var varFecha1 = new Date(varFechaHabile1 + " 00:00:00");
			var varRespuesta = "";
			console.log(varFecha1);
					...
					reader.onload = function (evn) {
						try {
							...
							if (varOpcionValidarFacturaFechaEmision === "S") {
                                ...
                                $.ajax({
                                    type: "GET",
                                    url: this.varTableURL + "/" + this.varTableT_FAC + "?$format=json&$filter=US_RUC eq '" + usuarioLogin + "' and EM_RUC eq '" + this.varRucDeLaEmpresa + "' and ID_FACTURA eq '" + varNumFacturaValid + "'",
                                    success: function (response) {
										var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
										var lenghtV = oModelJSON.getData().length;
				...
				if (file && window.FileReader) {
					...
					reader.onload = function (evn) {
						try {
                            ...
                            var varVersionFormaPagoValidar2 = oModel2.getProperty("/cac:PaymentTerms/1/cbc:PaymentMeansID");
							console.log(varVersionFormaPagoValidar1);
                            console.log(varVersionFormaPagoValidar2);
							if (varVersionFormaPagoValidar1 !== "" && varVersionFormaPagoValidar1 !== undefined) {
								varVersionFormaPagoValidar = varVersionFormaPagoValidar1;
							} else if (varVersionFormaPagoValidar2 !== "" && varVersionFormaPagoValidar2 !== undefined) {
								varVersionFormaPagoValidar = varVersionFormaPagoValidar2;
							}*/

                            var varVersionFormaPagoValidar = "";
                            this.varContador1 = 0;
                            var varContador2 = 0;
                            var varVersionFormaPagoValidar1 = "";
                            while (this.varContador1 <= 5) {

                                try {
                                    var varVersionFormaPagoValidar11 = oModel2.getProperty("/cac:PaymentTerms/" + varContador2 + "/cbc:ID"); 
									var varVersionFormaPagoValidar12 = oModel2.getProperty("/n1:PaymentTerms/" + varContador2 + "/n2:ID"); //OKOKOK2
									if (varVersionFormaPagoValidar11 !== "") {
										varVersionFormaPagoValidar1 = varVersionFormaPagoValidar11;
									} else if (varVersionFormaPagoValidar12 !== "") {
										varVersionFormaPagoValidar1 = varVersionFormaPagoValidar12;
									}
                                } catch (error) {
                                    this.varContador1 = 6;
                                }
                                ...
                                if (varVersionFormaPagoValidar1 === "FormaPago" || varVersionFormaPagoValidar1 === "formaPago") {
                                    this.varContador1 = 6;
                                }
                                console.log(varVersionFormaPagoValidar1);
                                this.varContador1++; 
                            ...
                            varVersionFormaPagoValidar = varVersionFormaPagoValidar1;
                            console.log(varVersionFormaPagoValidar);
												...
												if (lenghtV === 0) { // quitar || lenghtV !== 0 20200304
													if (varVersionUBLValidar === "2.1") {
														...
														if (varRUCEmisor.trim().toString() === usuarioLogin.trim().toString()) {
                                                            ...
                                                            //Begin I@MM-01/03/2022-Ticket-2022-581
                                                            var varDetraccValidar = "";
                                                            this.varContadorDetracc1 = 0;
                                                            var varContadorDetracc2 = 0;
                                                            this.varContadorDetracc3 = 0;
                                                            var varDetraccValidar1 = "";
                                                            while (this.varContadorDetracc1 === 0 && this.varContadorDetracc3 <= 5) {
                                                                try {
                                                                    varDetraccValidar1 = oModel2.getProperty("/cac:PaymentTerms/" + varContadorDetracc2 + "/cbc:PaymentPercent");
                                                                } catch (error) {
                                                                    this.varContadorDetracc1 = 1;
                                                                }

                                                                console.log(varDetraccValidar1);

                                                                if (this.varContadorDetracc3 === 5) {
                                                                    varDetraccValidar1 = "";
                                                                }

                                                                varContadorDetracc2++;
                                                                this.varContadorDetracc3++;
                                                            }
                                                            oModel.setProperty("/varGlobalListDetraccValidar1", varDetraccValidar1);
                                                            //End I@MM-01/03/2022-Ticket-2022-581

															//var varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentMeans/cbc:PaymentMeansCode");
															//var varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentTerms/cbc:PaymentMeansID");
                                                            var varCodTipoDetraccion = varDetraccValidar1;
															/*if (varCodTipoDetraccion !== "") { // MAURO 20211014
																//varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentMeans/cbc:PaymentMeansCode");
																varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentTerms/cbc:PaymentMeansID");
																var varTamCodigoDetr = oModel.getProperty("/codigoDetr").length;
															...
															this.oGlobalCodTipoDetraccion = ""; // MAURO 20211014
                                                            this.oGlobalCodMontoDetraccion = "0";
															//this.oGlobalCodTipoDetraccion = "17"; // MAURO 20211014
															for (var i = 0; i < oTblDetraccion.length; i++) {
																console.log(oTblDetraccion[i].NUMTP + " - " + varCodTipoDetraccion);
																if (oTblDetraccion[i].NUMTP === varCodTipoDetraccion) {
																	this.oGlobalCodTipoDetraccion = varCodTipoDetraccion;
                                                                    //this.oGlobalCodMontoDetraccion = this.funObtenerIndMonto(varCodTipoDetraccion); //20220511
																}
															}
															...
															if (varRUCReceptor === usuarioRuc) {
																...
																if (realizar) {
                                                                    ...
                                                                    var varSignSUNAT = oModel2.getProperty("/cac:Signature/cac:DigitalSignatureAttachment/cac:ExternalReference/cbc:URI"); //I@MM-01/03/2022-Ticket-2022-581
                                                                    this.oGlobalSignSUNAT = varSignSUNAT; //I@MM-01/03/2022-Ticket-2022-581
																	...
																	this.getView().byId("idImporteTotalIGV").setValue("     " + varTotatlSinIGV.toFixed(2));
																	this.getView().byId("idValorReferencial").setValue("     " + ""); //I@MM-21/12/2021-Ticket-2021-999
                                                                    this.oGlobalCodMontoTotal = varImporteTotal; //I@MM-01/03/2022-Ticket-2022-581

																	//I@MM-21/12/2021-Ticket-2021-999
																	oModel.setProperty("/valGlobalImporteTotalIGV", varTotatlSinIGV.toFixed(2));
                                                                    var varImporteTotal2 = parseFloat(varImporteTotal, 10);
                                                                    oModel.setProperty("/valGlobalImporteTotalConIGV", varImporteTotal2.toFixed(2));
																	/*var varSelecTipoCarga = this.getView().byId("idComboTipo").getSelectedItem().getKey();
																	if (varSelecTipoCarga === "H") {
																		console.log("H");
																		var oVectorlistServValReferencial = oModel.getProperty("/tblServDetraccion");
		...
		metDialogValorReferencial: function () {
            ...
            var varValorImporteTotalConIGV = oModel.getProperty("/valGlobalImporteTotalConIGV");
			...
			var dialogMensaje = new sap.m.Dialog({
				...
				content: [
					...
					new sap.m.Input("idValorReferencial2", {
						value: varValorImporteTotalConIGV,
				...
				beginButton: new sap.m.Button({
					press: function () {
						...
						var varValor1 = parseFloat(varValorImporteTotalConIGV.trim(), 10).toFixed(2);
						var varValor2 = parseFloat(sap.ui.getCore().byId("idValorReferencial2").getValue().trim(), 10).toFixed(2);
						console.log(varValor1 + " > " + varValor2);
						if (varValor2 === "" || varValor2 === undefined || varValor2 === NaN || varValor2 === "NaN") {
							varValor2 = "";
						}

                        varValor1 = parseFloat(varValor1);
                        varValor2 = parseFloat(varValor2);
						...
						if (varValor1 > varValor2) {
							var dialogA = new sap.m.Dialog({
								...
								state: "Warning",
								content: new sap.m.Text({
									text: "El valor importe con IGV es mayor que el valor referencial."
								}),
								beginButton: new sap.m.Button({
									text: "OK",
									type: "Accept",
									press: function () {
										this.getView().byId("idValorReferencial").setValue("     " + varValor1);
		...
		btnUdpLabelValorReferencial: function () {
            ...
            var varValorImporteTotalConIGV = this.getView().byId("idImporteTotal").getValue().trim();
			...
			var dialogMensaje = new sap.m.Dialog({
				...
				beginButton: new sap.m.Button({
					press: function () {
						...
						var varValor1 = parseFloat(varValorImporteTotalConIGV, 10).toFixed(2);
						var varValor2 = parseFloat(sap.ui.getCore().byId("idValorReferencial2").getValue().trim(), 10).toFixed(2);
						console.log(varValor1 + " > " + varValor2);
		...
		btnFacturarMesage: function () {
            ...
            var itemComboboxRetrac2 = this.getView().byId("idCodigoDetra2").getSelectedItem().getKey();/// 20220425
			/// 20200608
			var oThis = this;
			var oView = oThis.getView();
			var oModel = oView.getModel("myParam");
			...
			varTamListDocumentos = 1 * parseInt(varTamListDocumentos, 10);
            ...
            if (itemComboboxRetrac===itemComboboxRetrac2) { // ludwig 20220425
				if (itemComboboxRetrac === "N.A." || itemComboboxRetrac === "NA" && itemComboboxRetrac===itemComboboxRetrac2) { // MAURO 20211014
                    textDecision = "NO";
                } else {
                    textDecision = "SI";
                }
				...
				MessageBox.warning("Favor comunicarse con el área de contabilidad  o enviar un correo a contabilidad@redondos.com.pe para validar su concepto de detracción");
			}
		...
		btnFacturar: function () {
			...
			if (itemSeleccionado !== null && itemSeleccionado !== undefined) {
				...
				var totalValReferencial = this.getView().byId("idValorReferencial").getValue(); //I@MM-21/12/2021-Ticket-2021-999
				...
				if (halloError === "") {
                    ...
                    $.ajax({
                        type: "GET",
                        url: this.varTableURL + "/" + this.varTableT_FAC + "?$format=json&$filter=EM_RUC eq '" + this.varRucDeLaEmpresa + "' and US_RUC eq '" + usuarioLogin + "' and ID_FACTURA eq '" + paginaCard.description + "'",
						success: function (response) {
							var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
							var lenghtV = oModelJSON.getData().length;
							if (lenghtV === 0) {
                                ...
                                var oHeaders = { 'X-CSRF-Token': this.token };
                                var aArchivos = model.getProperty("/listTablaDocumentos");
                                for (var i1 = 0; i1 < aArchivos.length; i1++) {
                                    data = {
                                        'propertyId[0]': 'cmis:objectTypeId',
                                        'propertyValue[0]': 'cmis:document',
                                        'propertyId[1]': 'cmis:name',
                                        'propertyValue[1]': aArchivos[i1].clistTabDocuNombreArch,
                                        'cmisaction': 'createDocument'
                                    };
                                    formData = new FormData();
                                    formData.append('datafile', aArchivos[i1].clistTabDocuArchivo);
								...
								SOHeader.FEC_PAGO = "";
								SOHeader.FEC_TEN = totalValReferencial.trim(); //I@MM-21/12/2021-Ticket-2021-999
                                ...
                                var oVectorJSON = JSON.stringify(SOHeader);
                                var oHeaders = { 'X-CSRF-Token': this.token };
                                $.ajax(this.varTableURL + "/" + this.varTableT_FAC, {
                                    headers: oHeaders,
                                    data: oVectorJSON,
                                    type: "POST",
                                    contentType: "application/json",
                                    ...
                                    success: function (data) {
										...
										for (var a = 0; a < listItemDetalleFactura.length; a++) {
											...
											for (var b = 0; b < itemsOC.length; b++) {
                                                ...
                                                if (itemsOC[b].DE_DESCRIPCION !== "" && itemsOC[b].DE_DESCRIPCION !== null && itemsOC[b].DE_DESCRIPCION !== undefined) {
													llaveItem.DESCRIPCION = itemsOC[b].DE_DESCRIPCION.substring(0, 99);
												} else {
													llaveItem.DESCRIPCION = "";
												}
                                                ...
                                                var oVectorJSON = JSON.stringify(llaveItem);
                                                var oHeaders = { 'X-CSRF-Token': this.token };
                                                $.ajax(this.varTableURL + "/" + this.varTableT_FAC_POS, {
                                                    headers: oHeaders,
                                                    data: oVectorJSON,
                                                    type: "POST",
                                                    contentType: "application/json",
                                                    ...
                                                    success: function (data) {
                                                        ...
                                                        var oVectorJSON = JSON.stringify(llaveActualizar);
                                                        var oHeaders = { 'X-CSRF-Token': this.token }
                                                        $.ajax(this.varTableURL + texto, {
                                                            type: 'PATCH',
                                                            data: oVectorJSON,
                                                            headers: oHeaders,
                                                            contentType: "application/json",
                                            ...
                                            var oVectorJSON = JSON.stringify(llavePos);
                                            var oHeaders = { 'X-CSRF-Token': this.token };
                                            $.ajax(this.varTableURL + "/" + this.varTableT_FAC_DET, {
                                                headers: oHeaders,
                                                data: oVectorJSON,
                                                type: "POST",
                                                contentType: "application/json",
										...
										for (var i = 0; i < varTabla.length; i++) {
											...
											console.log(varDocItemArchivo);
                                            console.log(varDocItemNombre);
        ...
        funSubriDocumento: function (archivo, nombre, llave, date, time) {
            ...
            var oHeaders = { 'X-CSRF-Token': this.token };
            $.ajax(this.varTableURL + "/" + this.varTableT_DOC, {
                headers: oHeaders,
                data: oVectorJSON,
                type: "POST",
                contentType: "application/json",
		...
		funSubriDocumento2: function (archivo, nombre, llave, date, time) {
			var oThis = this;
			var oView = oThis.getView();
			var oModel = oView.getModel("myParam");
            var oHeaders = { 'X-CSRF-Token': this.token };
            ...
            var oVectorJSON = JSON.stringify(formData);
				...
				for (var d1 = 0; d1 < listDocAdjuntarFac.length; d1++) {
					...
					if (oConsultaDoc.length > 1 && oConsultaDoc != false) {
						...
						if (oDeleteDocRepo == "V") {
							...
							/*oModelOData.remove(documento, {*/
                            var oHeaders = { 'X-CSRF-Token': this.token }
                            $.ajax(this.varTableURL + documento, {
                                type: 'DELETE',						
                                headers: oHeaders,
                                contentType: "application/json",
                                dataType: "json",
									...
									if (this.varContTabla2 === listDocAdjuntarFac.length) {
										...
										if (listItemFacturas.length === 0) {
											...
											if (listItemFacturaPosicion.length === 0) {
                                                ...
                                                var oHeaders = { 'X-CSRF-Token': this.token }
                                                $.ajax(this.varTableURL + ordenCompra, {
                                                    type: 'DELETE',						
                                                    headers: oHeaders,
                                                    contentType: "application/json",
                                                    dataType: "json",	
												...
												for (var d4 = 0; d4 < listItemFacturaPosicion.length; d4++) {
													if (listItemFacturaPosicion[d4].US_RUC != undefined) { //GM26102021V2 incio
														...
														if (varTipoCargaDefinir === "S" || varTipoCargaDefinir === "H" || varTipoCargaDefinir === "D") {
                                                            ...
                                                            var oVectorJSON = JSON.stringify(llaveActualizar);
                                                            var oHeaders = { 'X-CSRF-Token': this.token }
                                                            $.ajax(this.varTableURL + actOrdenCompra, {
                                                                type: 'PATCH',
                                                                data: oVectorJSON,
                                                                headers: oHeaders,
                                                                contentType: "application/json",
                                                                ...
                                                                success: function (response) {
																	...
																	if (this.varContTabla4 === listItemFacturaPosicion.length) {
																		...
																		for (var d5 = 0; d5 < listItemFacturaPosicion.length; d5++) {
                                                                            ...
                                                                            var oHeaders = { 'X-CSRF-Token': this.token }
                                                                            $.ajax(this.varTableURL + detOrdenCompra, {
                                                                                type: 'DELETE',						
                                                                                headers: oHeaders,
                                                                                contentType: "application/json",
                                                                                dataType: "json",	
		...
		updateServicioSunat: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_SERVICIOS + "?$format=json&$filter=SERVICIO eq '" + "Validacion_CPE_SUNAT" + "'",
                success: function (response) {
					var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
					var lenghtV = oModelJSON.getData().length;
					if (lenghtV !== 0) {
			...
			if (this.varEstadoSunat1 === "X") {
                $.ajax(this.oBDURl + 'infosunat', {
                    ...
                    success: function (data, message, success) {
                        ...
                        console.log(data);
                        console.log(data.myResponse);
                        //console.log(data.myResponse.data);
                        ...
                        var oResultados = JSON.parse(data.myResponse);
                        ...
                        resultado = {};
                        switch (oResultados.condDomiRuc) {
                            case "00":
		...
		funDatosXMLInsertado: function () {
            ...
            var archivoValidar2 = file.name;
            console.log(archivoValidar2);
            /*$.ajax('/dms/' + archivoValidar2, { //20220606
                method: 'GET',
				success: function (response) {*/
					/*response = response.toString();
					response = response.split("<t1>");
		...
		btnAnadirDocSus: function () {
			...
			if (varSelecTipoCarga === "S") {
				varDocumentosObli = "( 01 ó 08 - xml ) y 01 - pdf"; // MAURO 20211014 //I@MM-17/01/2022-Ticket-2022-538
			...
			var dialog = new sap.m.Dialog({
				...
				content: [
					...
					new sap.ui.unified.FileUploader("idDocumentos", {
						...
						change: function (e) {
							sap.ui.getCore()._file = e.getParameter("files") && e.getParameter("files")[0];
                            this._fileDS = sap.ui.getCore()._file; //I@MM-17/01/2022-Ticket-2022-538
							sap.ui.getCore().byId("idAceptar").setEnabled(true);
						}.bind(this), //I@MM-17/01/2022-Ticket-2022-538
									...
									if (varContadorValidor === 0) {
                                        ...
                                        $.ajax('/dms/' + archivoValidar2, {
											...
											success: function (response) {
                                                ...
                                                //Begin I@MM-17/01/2022-Ticket-2022-538
                                                if (vectortxt1[1].Dato === "CDR") {
                                                    //var oValidarTipoCDR = this.funValidarTipoCDR(this._fileDS);
                                                    var file2 = this._fileDS;
                                                    var oRespuesta = "N";

                                                    console.log(file2);
                                                    try {
                                                        if (file2 && window.FileReader) {
                                                            var reader = new FileReader();
                                                            reader.onload = function (evn) {
                                                                try {
                                                                    console.log(evn);
                                                                    ...
                                                                    oModel4.setXML(strCSV);
                                                                    console.log(oModel4);

                                                                    var varNumFacturaValid1 = "";
                                                                    var varNumFacturaValid11 = oModel4.getProperty("/cac:DocumentResponse/cac:Response/cbc:Description");
                                                                    var varNumFacturaValid12 = oModel4.getProperty("/ns3:DocumentResponse/ns3:Response/Description");
                                                                    if (varNumFacturaValid11 !== "" && varNumFacturaValid11 !== undefined) {
                                                                        varNumFacturaValid1 = varNumFacturaValid11;
                                                                    } else if (varNumFacturaValid12 !== "" && varNumFacturaValid12 !== undefined) {
                                                                        varNumFacturaValid1 = varNumFacturaValid12;
                                                                    }
                                                                    var varCRDTagFactura = "";
                                                                    var varCRDTagFactura1 = oModel4.getProperty("/cac:DocumentResponse/cac:DocumentReference/cbc:ID");
                                                                    var varCRDTagFactura2 = oModel4.getProperty("/ns3:DocumentResponse/ns3:DocumentReference/ID");
                                                                    var varCRDTagFactura3 = oModel4.getProperty("/cac:DocumentResponse/cac:Response/cbc:ReferenceID");
                                                                    if (varCRDTagFactura1 !== "" && varCRDTagFactura1 !== undefined) {
                                                                        varCRDTagFactura = varCRDTagFactura1;
                                                                    } else if (varCRDTagFactura2 !== "" && varCRDTagFactura2 !== undefined) {
                                                                        varCRDTagFactura = varCRDTagFactura2;
                                                                    } else if (varCRDTagFactura3 !== "" && varCRDTagFactura3 !== undefined) {
                                                                        varCRDTagFactura = varCRDTagFactura3;
                                                                    }
                                                                    var varCRDTagSerie = varCRDTagFactura.split("-")[0];
                                                                    var varCRDTagNumComp = varCRDTagFactura.split("-")[1];
                                                                    var varCRDTagRuc = "";
                                                                    var varCRDTagRucCom = "";
                                                                    var varCRDTagRuc1 = oModel4.getProperty("/cac:DocumentResponse/cac:IssuerParty/cac:PartyLegalEntity/cbc:CompanyID");
                                                                    var varCRDTagRuc2 = oModel4.getProperty("/ns3:DocumentResponse/ns3:IssuerParty/ns3:PartyLegalEntity/CompanyID");
                                                                    var varCRDTagRuc3 = oModel4.getProperty("/cac:Signature/cac:SignatoryParty/cac:PartyIdentification/cbc:ID");
                                                                    var varCRDTagRuc4 = oModel4.getProperty("/cac:SenderParty/cac:PartyIdentification/cbc:ID");
                                                                    var varCRDTagRuc5 = oModel4.getProperty("/cac:ReceiverParty/cac:PartyIdentification/cbc:ID");
                                                                    var varOpcion11 = 0;
                                                                    if (varCRDTagRuc1 !== "" && varCRDTagRuc1 !== undefined) {
                                                                        varCRDTagRuc = varCRDTagRuc1;
                                                                    } else if (varCRDTagRuc2 !== "" && varCRDTagRuc2 !== undefined) {
                                                                        varCRDTagRuc = varCRDTagRuc2;
                                                                    } else if (varCRDTagRuc3 !== "" && varCRDTagRuc3 !== undefined) {
                                                                        varCRDTagRuc = varCRDTagRuc3;
                                                                    }
                                                                    
                                                                    if (varCRDTagRuc4 !== "" && varCRDTagRuc4 !== undefined && varCRDTagRuc4 === docRUCComp) {
                                                                        varCRDTagRucCom = varCRDTagRuc4;
                                                                        varOpcion11 = 1;
                                                                    } else if (varCRDTagRuc5 !== "" && varCRDTagRuc5 !== undefined && varCRDTagRuc5 === docRUCComp) {
                                                                        varCRDTagRucCom = varCRDTagRuc5;
                                                                        varOpcion11 = 1;
                                                                    }
                                                                    
                                                                    if (varOpcion11 === 1) {
                                                                        varCRDTagRuc = varCRDTagRucCom;
                                                                    }
                                                                    console.log("varNumFacturaValid1: " + varNumFacturaValid1);
                                                                    console.log("varCRDTagSerie: " + varCRDTagSerie);
                                                                    console.log("varCRDTagNumComp: " + varCRDTagNumComp);
                                                                    console.log("varCRDTagNumComp: " + parseInt(varCRDTagNumComp, 10));
                                                                    console.log("varCRDTagRuc: " + varCRDTagRuc);
                                                                    console.log("docSerieComp: " + docSerieComp);
                                                                    console.log("docNumeroComp: " + docNumeroComp);
                                                                    console.log("docNumeroComp: " + parseInt(docNumeroComp, 10));
                                                                    console.log("docRUCComp: " + docRUCComp);

                                                                    var oResp1 = varNumFacturaValid1.includes('ha sido aceptada');
                                                                    var oResp2 = varNumFacturaValid1.includes('ha sido aceptado');
                                                                    var oResp3 = varNumFacturaValid1.includes('ha sido ACEPTADA');
                                                                    var oResp4 = varNumFacturaValid1.includes('ha sido ACEPTADO');
                                                                    
                                                                    console.log(oResp1);
                                                                    console.log(oResp2);
                                                                    console.log(oResp3);
                                                                    console.log(oResp4);
                                                                    if (varCRDTagSerie === docSerieComp && parseInt(varCRDTagNumComp, 10) === parseInt(docNumeroComp, 10) && varCRDTagRuc === docRUCComp) {
                                                                        if (oResp1 || oResp2 || oResp3 || oResp4) {
                                                                            oRespuesta = "S";
                                                                        } else {
                                                                            oRespuesta = "N";
                                                                        }
                                                                    ...
                                                                    //Begin I@MM-17/01/2022-Ticket-2022-538
                                                                    console.log(vectortxt1[1].Dato);
                                                                    console.log(oRespuesta);
                                                                    if (oRespuesta === "S") { } else {
                                                                        llave.clistTabDocuEstado = "CDR no aceptado";
                                                                        varMenssageError = "El CDR del documento XML no es aceptado";
                                                                        varContError++;
                                                                    }

                                                                    //End I@MM-17/01/2022-Ticket-2022-538
                                            ...
                                            error: function (response) {
                                                console.log(response); //20220606
                                                console.log(response.status); //20220606
				...
				beginButton: new sap.m.Button({
					...
					type: 'Emphasized',
					press: function () {
                        var comboDetra2 = this.getView().byId("idCodigoDetra2");//I@LL-06/05/2022-Ticket-2022-0602
                        comboDetra2.getBinding("items").refresh(true);
                        var firstItem2 = comboDetra2.getItems()[0];
                        comboDetra2.setSelectedItem(firstItem2, true);
						var vartamtblDocOK = oModel.getProperty("/listTablaDocumentos").length;
							...
							if (varContNoEstado === 0) {
								if (this.getView().byId("idComboTipo").getSelectedItem().getKey() === "S") {
									...
									for (var k = 0; k < vartamtblDocOK; k++) {
                                        ...
                                        /*else if (opcFiltro === "CDR" && opcFiltroExtension === "text/xml") { //I@MM-17/01/2022-Ticket-2022-538
											contTipo = contTipo + 1;
										}*/
									}

									if (contTipo < 2) { // MAURO 20211014 //I@MM-17/01/2022-Ticket-2022-538
									...
									if (contTipo < 2) { //I@MM-17/01/2022-Ticket-2022-538
