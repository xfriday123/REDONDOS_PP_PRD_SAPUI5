...
], function (mobileLibrary, Controller, Fragment) {
	"use strict";
    var URLHelper = mobileLibrary.URLHelper;
	return Controller.extend("nsrdos.uiportalprov.controller.Vista_Menu_Principal", {
		...
		_onRouteMatched: function () {
            ...
            this.getView().byId("productInput").setVisible(false); //MM-20250203
            this.getView().byId("idListaProveedores").setVisible(false); //MM-AJUSTES-20240923 //MM-20250203
                    ...
                    if (varopcValidarExisteRUC === "S") {
                        ...
                        for (var k = 0; k < varTamCodigoRucProv.length; k++) {
                            if (parseInt(oModel.getProperty("/listaProveedoresRUC/" + k + "/codigo"), 10) === parseInt(varTamCodigoRucProvLogin, 10)) {
                                ...
                                productInput.setValue(valor2 + " - " + valor1); //MM-20250203
                            }
                        }

                        this.getView().byId("idRUCProveedor").setEditable(false);
                        this.getView().byId("idRUCProveedor").setVisible(false);
                        this.getView().byId("productInput").setVisible(true); //MM-20250203
                        this.getView().byId("idListaProveedores").setVisible(true); //MM-AJUSTES-20240923 //MM-20250203
                        this.getView().byId("idRUCProveedorNoAdmin1").setVisible(false);
                        this.getView().byId("idRUCProveedorNoAdmin2").setVisible(false);
                    } else {
                        this.getView().byId("idRUCProveedor").setEditable(false);
                        this.getView().byId("idRUCProveedor").setVisible(false); //MM-20250203
                        this.getView().byId("productInput").setVisible(false); //MM-20250203
                        this.getView().byId("idListaProveedores").setVisible(false); //MM-AJUSTES-20240923 //MM-20250203
                        this.getView().byId("idRUCProveedorNoAdmin1").setVisible(true); //MM-20250203
...
/*                                     files.forEach(function (file) {k
		...
		onAfterRendering: function (oEvent) {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, request) {
                    ...
                    this.getView().byId("idRUCProveedor").addStyleClass("styleRUCProveedor"); //MM-20250203
        ...
        _dgShowAnuncio: function () {
            // var oModel = this.getView().getModel("myParam"); j
            var oView = this.getView();
            if (!this.dgAnuncio) { this.dgAnuncio = sap.ui.xmlfragment("iddgAnuncio", "nsrdos.uiportalprov.view.fragments.dgAnuncio", this) };
		...
		handleClose: function (oEvent) {
			...
			if (aContexts.length) {
				...
				var productInput = this.getView().byId("productInput");
				productInput.setValue(valor2 + " - " + valor); //MM-20250203
        ...
        btnListaProveedores: function () { //MM-AJUSTES-20240923
            ...
            if (!this.pDialog_cliente) {
                this.pDialog_cliente = Fragment.load({
                    id: oView.getId(),
                    name: "nsrdos.uiportalprov.view.fragments.dgDialogAyudaCliente",
                    controller: this
        ...
        getDataResFacReg1: function() {

            var e = this.getView();

            var t = e.getModel("myParam");

            e.setModel(t);

            var a = "" + this.varTableURL + "/";

            var i = new sap.ui.model.odata.v2.ODataModel(a,true);

            var s = t.getProperty("/usuarioLogin");

            var o = t.getProperty("/usuarioRuc");
            ...
            n = new sap.ui.model.Filter("US_RUC",sap.ui.model.FilterOperator.EQ,s);

            r.push(n);

            n = new sap.ui.model.Filter("EM_RUC",sap.ui.model.FilterOperator.EQ,o);

            r.push(n);

            d = new sap.ui.model.Sorter("FC_FEC_REGISTRO",false);

            l.push(d);

            var c = this;

            var g = c.getView().getModel("myParam");

            this.oGlobalStop10 = "S";

            this.oGlobalIteracion0 = "0";

            g.setProperty("/oListaVectorCabeceraDetalle", []);
            ...
            while (this.oGlobalStop10 === "S") {
                ...
                $.ajax({
                    ...
                    url: this.varTableURL + "/" + this.varTableT_FAC + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=US_RUC eq '" + s + "' and EM_RUC eq '" + o + "'",
                    ...
                    success: function(e) {
                        ...
                        if (a.length !== 0) {
                            ...
                            var i = g.getProperty("/oListaVectorCabeceraDetalle");
                            ...
                            if (a.length !== 0) {
                                ...
                                g.setProperty("/oListaVectorCabeceraDetalle", i)
                            ...
                            console.log(g.getProperty("/oListaVectorCabeceraDetalle"));

                            var r = parseInt(this.oGlobalIteracion0, 10);

                            r = r + 1e3;

                            this.oGlobalIteracion0 = r.toString();
                        ...
                        if (this.oEntrar === "S") {
                            ...
                            var l = g.getProperty("/oListaVectorCabeceraDetalle");

                            var n = new sap.ui.model.json.JSONModel(l);

                            var d = n.getData().length;
                            ...
                            for (var o = 0; o < d; o++) {
                                ...
                                b.FC_FEC_REGISTRO = n.getData()[o].FC_FEC_REGISTRO;

                                c.push(b)

                            }

                            t.setProperty("/listT_FAC_Details", c);

                            this.getView().byId("idItem7Registro").setValue(c.length);

                            this.getView().byId("idItem7Registro").setValueColor("Good");

                            for (var u = 0; u < c.length; u++) {

                                if (c[u].FC_FEC_REGISTRO === "") {
