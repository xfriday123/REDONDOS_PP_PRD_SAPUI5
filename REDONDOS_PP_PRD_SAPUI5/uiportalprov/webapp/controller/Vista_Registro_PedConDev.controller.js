sap.ui.define([
	...
	return Controller.extend("nsrdos.uiportalprov.controller.Vista_Registro_PedConDev", {
		...
		onAfterRendering: function() {
            ...
            while (this.oGlobalStop10 === "S") {
				$.ajax({
					type: "GET",
					async: false,
					url: this.varTableURL + "/" + this.varTableT_OC_DET + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=US_RUC eq '" + varUsuario + "' and EM_RUC eq '" + this.varRucDeLaEmpresa + "' and DE_FLAC ne '" + "x" + "' and DE_FLAC ne '" + "X" + "'",
			...
			$.ajax({
				type: "GET",
				url: this.varTableURL + "/" + this.varTableT_CON_DET + "?$format=json&$filter=US_RUC eq '" + varUsuario + "' and EM_RUC eq '" + this.varRucDeLaEmpresa + "'",
				success: function(response) {
			...
			this.getView().byId("idDocReferencia").setValue("");
		...
		onInit: function() {
			...
			var oThis = this;
			//oThis.getRouter().getRoute("Vista_Registro_PedConDev").attachMatched(this._onRouteMatched, this);
			...
			oTableItem.addColumn(new sap.ui.table.Column({
				...
				template: new sap.m.CheckBox({
					...
					select: function(evt) {
						...
						var varDocMaterial = objeto.DE_DOC_MATERIAL;
						...
						if (varContadorDiferente === 0) {
							if (objeto.selectItem) {
								for (var i = 0; i < varTam2; i++) {
									if (i !== vEliLaborID) {
										if (listaValesIngresoConsig[i].DE_DOC_MATERIAL === varDocMaterial && listaValesIngresoConsig[i].DE_MONEDA ===
											varMonedaOCAnt) {
							...
							evt.getSource().setSelected(false);
							sap.m.MessageToast.show("Solo se puede asignar importes con diferente orden de compra");
						}
			...
			var dialogMensaje = new sap.m.Dialog({
				...
				endButton: new sap.m.Button({
					press: function() {
						dialogMensaje.close();
						if (llave === "D") {
							this.getView().byId("idComboTipo").setSelectedKey("D");
						}
					}.bind(this),
					text: "Cancelar"
				}),
		...
		verificarAsignaciónPosFactura: function() {
			...
			if (listaItem.length !== 0) {
				...
				if (asignado) {
					...
					this.getView().byId("idFacturar").setEnabled(false); //////////////////////// Poner false
				}
			} else {
				this.getView().byId("idValidacionFacAsig").setText("Sin items a asignar.");
				this.getView().byId("idValidacionFacAsig").setType("Information");
		...
		oDialogOrdenDeCompra: function(varPosicionFactura, varCodigoFactura, varValortotalNetoFactura, varDescripcionFactura,
			...
			var lista = new sap.m.List({
				...
				select: function(evt) {
					...
					$.ajax({
						type: "GET",
						url: this.varTableURL + "/" + this.varTableT_OC_DET + "?$format=json&$filter=DE_NUMERO_ORDEN eq '" + vlistOrdenCompraNum + "' and DE_FLAC ne '" + "x" + "' and DE_FLAC ne '" + "X" + "'",
						success: function(response) {
							try {
								var realizar = false;
								var realizar2 = false;
				...
				if (file && window.FileReader) {
					...
					reader.onload = function(evn) {
						try {
							...
							if (varNumFacturaValid === "") {
								varNumFacturaValid = oModel2.getProperty("/n2:ID");
							}

							var varValidacionExiste = oModel2.getProperty("/cac:CreditNoteLine/0/cbc:ID");
							if (varValidacionExiste === "") {
								varValidacionExiste = oModel2.getProperty("/n1:CreditNoteLine/0/n2:ID");
							}

							//Begin I@MM-24/12/2021-Ticket-2021-525
							/*var varVersionFormaPagoValidar = "";
							var varVersionFormaPagoValidar1 = oModel2.getProperty("/cac:PaymentTerms/cbc:PaymentMeansID");
							...
							while (this.varContador1 === 0) {

							    try {
							        varVersionFormaPagoValidar1 = oModel2.getProperty("/cac:PaymentTerms/" + varContador2 + "/cbc:ID");
							    } catch (error) {
							        this.varContador1 = 1;
							    }
							...
							if (varMatrizDiasHabiles[0].ESTADO === "X") {
								...
								if (varFecEmisionValidarFactura === "") {
									varFecEmisionValidarFactura = oModel2.getProperty("/n2:IssueDate");
								}
								...
								if (varOpcionValidarFacturaFechaEmision === "S") {
									...
									$.ajax({
										...
										success: function(response) {
											var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
											var lenghtV = oModelJSON.getData().length;
											if (varValidacionExiste !== "") { //2020305
												if (lenghtV === 0) { //////////////// quitar  || lenghtV !== 0
													...
													if (varVersionUBL === "") {
														varVersionUBL = oModel2.getProperty("/n2:UBLVersionID");
													}
													var varNumFactura = oModel2.getProperty("/cbc:ID");
													if (varNumFactura === "") {
														varNumFactura = oModel2.getProperty("/n2:ID");
													}
													var varFecEmision = oModel2.getProperty("/cbc:IssueDate");
													if (varFecEmision === "") {
														varFecEmision = oModel2.getProperty("/n2:IssueDate");
													}
													...
													if (varNomComercial === "") {
														varNomComercial = oModel2.getProperty("/cac:AccountingSupplierParty/n1:Party/n1:PartyName/n2:Name");
													}
													var varDocDeReferencia = "";
													var varDocDeReferencia1 = oModel2.getProperty("/cac:DiscrepancyResponse/cbc:ReferenceID");
													if (varDocDeReferencia1 === "") {
														varDocDeReferencia1 = oModel2.getProperty("/n1:DiscrepancyResponse/n2:ReferenceID");
													}
													var varDocDeReferencia2 = oModel2.getProperty("/cac:BillingReference/cac:InvoiceDocumentReference/cbc:ID");
													if (varDocDeReferencia2 === "") {
														varDocDeReferencia1 = oModel2.getProperty("/n1:BillingReference/n1:InvoiceDocumentReference/n2:ID");
													}
													if (varDocDeReferencia1 === "") {
														varDocDeReferencia = varDocDeReferencia2;
													} else {
														varDocDeReferencia = varDocDeReferencia1;
													}
													this.getView().byId("idDocReferencia").setValue(varDocDeReferencia.toString());
													...
													if (rucEmisor2 === "") {
														rucEmisor2 = oModel2.getProperty("/cac:AccountingSupplierParty/n1:Party/n1:PartyIdentification/n2:ID");
													}
													var rucEmisor3 = oModel2.getProperty("/cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID");
													if (rucEmisor3 === "") {
														rucEmisor3 = oModel2.getProperty("/cac:AccountingSupplierParty/n2:CustomerAssignedAccountID");
													}
													var varRUCEmisor = "";
													...
													try {
														if (varRUCEmisor.trim().toString() === usuarioLogin.trim().toString()) {
															...
															if (rucReceptor2 === "") {
																rucReceptor2 = oModel2.getProperty("/cac:AccountingCustomerParty/n1:Party/n1:PartyIdentification/n2:ID");
															}
															var rucReceptor3 = oModel2.getProperty("/cac:AccountingCustomerParty/cbc:CustomerAssignedAccountID");
															if (rucReceptor3 === "") {
																rucReceptor3 = oModel2.getProperty("/n1:AccountingCustomerParty/n2:CustomerAssignedAccountID");
															}
															var varRUCReceptor = "";
															...
															if (varRUCReceptor === usuarioRuc) {
																...
																if (recRazonSocial2 === "") {
																	recRazonSocial2 = oModel2.getProperty(
																...
																if (varMoneda === "") {
																	varMoneda = oModel2.getProperty("/n2:DocumentCurrencyCode");
																}

																//////////////////////////////////////////////////////////////////////////7
																var varCabeTotalImporte = oModel2.getProperty("/cac:LegalMonetaryTotal/cbc:PayableAmount");
																if (varCabeTotalImporte === "") {
																	varCabeTotalImporte = oModel2.getProperty("/n1:LegalMonetaryTotal/n2:PayableAmount");
																}
																var varCabeTotalIGV = oModel2.getProperty("/cac:TaxTotal/cbc:TaxAmount");
																if (varCabeTotalIGV === "") {
																	varCabeTotalIGV = oModel2.getProperty("/n1:TaxTotal/n2:TaxAmount");
																}
																...
																var realizarN1 = false;
																if (CreditNoteLine === "") {
																	CreditNoteLine = oModel2.getProperty("/n1:CreditNoteLine");
																	realizarN1 = true;
																}
																...
																if (realizar) {
																	while (cont === 1) {
																		try {
																			if (realizarN1) {
																				oModel2.getProperty("/n1:CreditNoteLine" + "/" + tamDetalleFactura + "/cbc:ID");
																			} else {
																	...
																	for (var j = 0; j < tamDetalleFactura; j++) {
																		...
																		if (varPosicion[j] === "") {
																			varPosicion[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n2:ID");
																		}

																		valor1Codigo = oModel2.getProperty(texto + "/" + j + "/cac:Item/cac:SellersItemIdentification/cbc:ID");
																		if (valor1Codigo === "") {
																			valor1Codigo = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n1:Item/n1:SellersItemIdentification/n2:ID");
																		}
																		...
																		if (varDescripcion[j] === "") {
																			varDescripcion[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n1:Item/n2:Description");
																		}
																		...
																		if (varUnidMedida[j] === "") {
																			varUnidMedida[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n2:CreditedQuantity" + "/@unitCode");
																		}
																		varCantidad[j] = oModel2.getProperty(texto + "/" + j + textoVariable);
																		if (varCantidad[j] === "") {
																			varCantidad[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n2:CreditedQuantity");
																		}
																		varAfectacionIGV[j] = oModel2.getProperty(texto + "/" + j +
																			"/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:Name");
																		if (varAfectacionIGV[j] === "") {
																			varAfectacionIGV[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j +
																		...
																		if (varPrecioUnitxItem[j] === "") {
																			varPrecioUnitxItem[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n1:Price/n2:PriceAmount");
																		}
																		varPrecioVentaxItem[j] = oModel2.getProperty(texto + "/" + j +
																			"/cac:PricingReference/cac:AlternativeConditionPrice/cbc:PriceAmount");
																		if (varPrecioVentaxItem[j] === "") {
																			varPrecioVentaxItem[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j +
																		...
																		if (varTotalIGVxItem[j] === "") {
																			varTotalIGVxItem[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n1:TaxTotal/n2:TaxAmount");
																		}
																		varValorVentaxItem[j] = oModel2.getProperty(texto + "/" + j + "/cbc:LineExtensionAmount");
																		if (varValorVentaxItem[j] === "") {
																			varValorVentaxItem[j] = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n2:LineExtensionAmount");
																		}
																		...
																		if (var2PorcentajeVal === "") {
																			var2PorcentajeVal = oModel2.getProperty("/n1:CreditNoteLine" + "/" + j + "/n1:TaxTotal/n1:TaxSubtotal/n1:TaxCategory/n2:Percent");
																		}
																	...
																	if (varTotalIGV === "") {
																		varTotalIGV = oModel2.getProperty("/n1:TaxTotal/n2:TaxAmount");
																	}
																	var varImporteTotal = oModel2.getProperty("/cac:LegalMonetaryTotal/cbc:PayableAmount");
																	if (varImporteTotal === "") {
																		varImporteTotal = oModel2.getProperty("/n1:LegalMonetaryTotal/n2:PayableAmount");
																	}
																	//Begin I@MM-16/02/2022-Ticket-2022-001
																	//var TotalIGV = oModel2.getProperty("/cac:TaxTotal/cac:TaxSubtotal/cbc:TaxAmount");
																	var TotalIGV = "";
																	this.oGlobalPararIGV = 0;
																	var oConteoIGV = 0;
																	while (this.oGlobalPararIGV === 0) {
																		try {
																			console.log(oConteoIGV);
																			var TotalIGVNombreTipo = oModel2.getProperty("/cac:TaxTotal/cac:TaxSubtotal/" + oConteoIGV + "/cac:TaxCategory/cac:TaxScheme/cbc:Name");
																			if (TotalIGVNombreTipo === "IGV") {
																				TotalIGV = oModel2.getProperty("/cac:TaxTotal/cac:TaxSubtotal/" + oConteoIGV + "/cbc:TaxAmount");
																				console.log(TotalIGV);
																				this.oGlobalPararIGV = 1;
																			} else {
																				var TotalIGV = oModel2.getProperty("/cac:TaxTotal/cac:TaxSubtotal/cbc:TaxAmount");
																				if (TotalIGV === "") {
																					TotalIGV = oModel2.getProperty("/n1:TaxTotal/n1:TaxSubtotal/n2:TaxAmount");
																				}
																			}
																			console.log(TotalIGV);
																			console.log(TotalIGVNombreTipo);
																		} catch (error) {
																			this.oGlobalPararIGV = 1;
																		}
																		oConteoIGV++;
																	}
																	//End I@MM-16/02/2022-Ticket-2022-001

																	var varTotatlSinIGV = parseFloat(varImporteTotal, 10) - parseFloat(TotalIGV, 10);
																	console.log(varImporteTotal);
																	console.log(TotalIGV);
																	console.log(varTotatlSinIGV);
																	console.log(oModel2.getProperty("/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:Name"));
												...
												var dialogValTipFac = new sap.m.Dialog({
													...
													icon: "sap-icon://error",
													content: new sap.m.Text({
														text: "La Factura que se intenta ingresar no es un tipo Nota de Créito(07)"
													}),
													beginButton: new sap.m.Button({
														text: "Aceptar",
														press: function() {
		...
		btnFacturar: function() {
			...
			if (afirmacion) {
					...
					$.ajax({
						...
						success: function(response) {
							...
							if (lenghtV === 0) {
								...
								SOHeader.TIPO_OPERACION = this.getView().byId("idDocReferencia").getValue();
											...
											for (var b = 0; b < itemsOC.length; b++) {
												...
												$.ajax(this.varTableURL + "/" + this.varTableT_FAC_POS, {
													...
													success: function(data) {
														...
														var texto = "/" + this.varTableT_OC_DET + "(EM_RUC='" + this.varRucDeLaEmpresa + "',US_RUC='" + data.US_RUC + "',DE_POSICION='" +
															data.DE_POSICION +
								...
								// Activar Flac
								/*var varTamSelectTbl = model.getProperty("/listSeleccionValesIngreso").length;
								var varSelectTbl = model.getProperty("/listSeleccionValesIngreso");
								for (var k = 0; k < varTamSelectTbl; k++) {
									var llaveSelect = {};
									llaveSelect.DE_FLAC = "X";
									oModel.update("/T_OC_DETs(DE_POSICION='" + varSelectTbl[k].DE_POSICION +
										"',DE_NUMERO_ORDEN='" + varSelectTbl[k].DE_NUMERO_ORDEN +
										"',DE_DOC_MATERIAL='" + varSelectTbl[k].DE_DOC_MATERIAL +
										"',US_RUC='" + varSelectTbl[k].US_RUC +
										"',DE_GUIA_REMISION='" + varSelectTbl[k].DE_GUIA_REMISION +
										"',DE_HOJA_ENTRADA='" + varSelectTbl[k].DE_HOJA_ENTRADA +
										"',DE_POS_DOC_MATERIAL='" + varSelectTbl[k].DE_POS_DOC_MATERIAL +
										"',EM_RUC='" + varSelectTbl[k].EM_RUC + "')", llaveSelect, {
											method: "PUT",
											success: function (data) {
												this.obtenerExitoOdata("Se Activo el Flac de todas las Ordenes de Compra Seleccionadas");
											}.bind(this),
											error: function (data) {
												this.obtenerErrorOdata("Error al Activar el Flac para todas las Ordenes de Compra");
											}.bind(this)
										});
								}*/
				...
				var dialog = new sap.m.Dialog({
					...
					state: "Warning",
					content: new sap.m.Text({
						text: "Se requiere un Documento de Referencia."
			...
			if (this.varEstadoSunat1 === "X") {
				$.ajax(this.oBDURl + 'infosunat', {
					...
					success: function(data, message, success) {
						...
						console.log(data.myResponse.data.estadoCp);
		...
		btnAnadirDocSus: function() {
			...
			if (varSelecTipoCarga === "D") {
				varDocumentosObli = "07 - xml y 07 - pdf"; //I@MM-17/01/2022-Ticket-2022-538
			}
			...
			var dialog = new sap.m.Dialog({
				...
				content: [
					...
					new sap.ui.unified.FileUploader("idDocumentos", {
						...
						change: function(e) {
							console.log("Aceptar");
							sap.ui.getCore().byId("idAceptarD").setEnabled(true); //GM26102021							
							sap.ui.getCore()._file = e.getParameter("files") && e.getParameter("files")[0];
					...
					new sap.m.Button("idAceptarD", { //GM26102021
						text: "Subir",
						width: "27%",
						type: "Emphasized",
						press: function() {
							...
							if (afirmacion) {
								...
								if (varTamanoDeArchivo !== 0) {
									sap.ui.getCore().byId("idAceptarD").setEnabled(false); //GM26102021			
														...
														if (file2 && window.FileReader) {
															...
															reader.onload = function(evn) {
																try {
																	...
																	console.log(varNumFacturaValid1);
																	console.log(varCRDTagSerie);
																	console.log(varCRDTagNumComp);
																	console.log(varCRDTagRuc);
																	console.log(docSerieComp);
																	console.log(docNumeroComp);
																	console.log(docRUCComp);
					...
					press: function() {
						...
						if (varContErrorBinario === 0) {
							if (varContNoEstado === 0) {
								if (this.getView().byId("idComboTipo").getSelectedItem().getKey() === "D") {
									...
									for (var k = 0; k < vartamtblDocOK; k++) {
										...
										if ((opcFiltro === "07") && opcFiltroExtension === "text/xml") {
											contTipo = contTipo + 1;
										} else if (opcFiltro === "07" && opcFiltroExtension === "application/pdf") {
											contTipo = contTipo + 1;
										}
