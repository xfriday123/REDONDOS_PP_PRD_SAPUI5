sap.ui.define([
    ...
    return Controller.extend("nsrdos.uiportalprov.controller.Vista_Reporte_Vale", {
        ...
        getUsuarioCorreo: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_USER + "?$format=json&$filter=US_USUARIO eq '" + varUsuario + "'",
                success: function (response) {
                    var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
                    ...
                    success: function (response) {
                        ...
                        if (this.oEntrar === "S") {
                            ...
                            /*oModel.read("/" + this.varTableT_OC_DET + "?$format=json", {
                                filters: filters1OC,*/
                                this.oGlobalStop11 = "S";
                                this.oGlobalIteracion1 = "0";
                                this.oEntrar1 = "N";
                                oModel9.setProperty("/oListaVectorOrdenDetalle", []);

                                while (this.oGlobalStop11 === "S") {
                                    $.ajax({
                                        type: "GET",
                                        async: false,
                                        ...
                                        success: function (response) {
                                            ...
                                            if (l !== 0) {
                                                var n = oModel9.getProperty("/oListaVectorOrdenDetalle");
                                                var p = {};
                                                ...
                                                oModel9.setProperty("/oListaVectorOrdenDetalle", n);
                                                var b = parseInt(this.oGlobalIteracion1, 10);

                                                b = b + 1000;

                                                this.oGlobalIteracion1 = b.toString();
                                                console.log(this.oGlobalIteracion1);
                                                this.oGlobalStop11 = "S"
                                            } else {
                                                this.oGlobalStop11 = "N";
                                                this.oEntrar1 = "S"
                                            }

                                            if (this.oEntrar1 === "S") {
                                                var ccc = oModel9.getProperty("/oListaVectorOrdenDetalle");
                                                var oModelJSONOC = new sap.ui.model.json.JSONModel(ccc);
                                                var lenghtVOC = oModelJSONOC.getData();
                                                ...
                                                for (var i = 0; i < lenghtV; i++) {
                                                    ...
                                                    llave.FLAC = "";
                                                    for (var j = 0; j < lenghtVOC.length; j++) {
                                                        if (oModelJSON.getData()[i].HOJA_ENTRADA === lenghtVOC[j].DE_HOJA_ENTRADA &&
                                                            oModelJSON.getData()[i].EM_RUC === lenghtVOC[j].EM_RUC &&
                                                            oModelJSON.getData()[i].DOC_MATERIAL === lenghtVOC[j].DE_DOC_MATERIAL &&
                                                            oModelJSON.getData()[i].US_RUC === lenghtVOC[j].US_RUC &&
                                                            oModelJSON.getData()[i].NUMERO_ORDEN === lenghtVOC[j].DE_NUMERO_ORDEN &&
                                                            oModelJSON.getData()[i].POSICION_ORDEN === lenghtVOC[j].DE_POSICION &&
                                                            lenghtVOC[j].DE_FLAC !== "") {
                                                            llave.FLAC = "X";
                                                        }
                                                    }
        ...
        getDataOrdenCompra: function () {
            ...
            $.ajax({
                type: "GET",
                url: this.varTableURL + "/" + this.varTableT_ORD + "?$format=json&$filter=EM_RUC eq '" + usuarioRuc + "' and US_RUC eq '" + usuarioLogin + "' and OC_NUMERO_ORDEN eq '" + varNumOrden + "'",
                success: function (responseC) {

                    // Cabecera
                    console.log(new sap.ui.model.json.JSONModel(responseC));
                    console.log(new sap.ui.model.json.JSONModel(responseC.results));
                    console.log(new sap.ui.model.json.JSONModel(responseC.value));
                    var oModelJSONC = new sap.ui.model.json.JSONModel(responseC.value);
                    var tamTablaC = oModelJSONC.getData().length;
                    var vectorC = [];
                    var llaveC = {};

                    for (var i = 0; i < tamTablaC; i++) {
                        if (oModel2.getProperty("/oGlobalTipoFechaAnio01") === oModelJSONC.getData()[i].OC_FECHA.substring(6, oModelJSONC.getData()[i].OC_FECHA.length)) {
                            llaveC = {};
                    ...
                    while (this.oGlobalStop10 === "S") {
                        $.ajax({
                            type: "GET",
                            async: false,
                            url: this.varTableURL + "/" + this.varTableT_ORD_DET + "?$format=json&$skiptoken=" + this.oGlobalIteracion0 + "&$filter=EM_RUC eq '" + usuarioRuc + "' and US_RUC eq '" + usuarioLogin + "' and OC_NUMERO_ORDEN eq '" + varNumOrden + "'",
                            success: function (responseD) {

                                var oDataHana = responseD.value;
                                ...
                                if (this.oEntrar === "S") {
                                    var oVectorCabeceraDetalle = oModel9.getProperty("/oListaVectorCabeceraDetalle");
                                    var oModelJSOND = new sap.ui.model.json.JSONModel(oVectorCabeceraDetalle);
            ...
            var oDialog = new sap.m.Dialog({
                ...
                beginButton: new sap.m.Button("idContinuar", {
                    ...
                    press: function () {
                        ...
                        $.ajax({
                            type: "GET",
                            url: this.varTableURL + "/" + this.varTableT_RESP_ORDEN + "?$format=json&$filter=EM_RUC eq '" + var_EM_RUC + "' and US_RUC eq '" + var_US_RUC + "' and OC_NUMERO_ORDEN eq '" + var_OC_NUMERO_ORDEN + "'",
                            success: function (response) {
                                var oModelJSON = new sap.ui.model.json.JSONModel(response.value);
                                var lenghtV = oModelJSON.getData().length;
                                ...
                                if (lenghtV !== 0) {
                                    ...
                                    var oHeaders = { 'X-CSRF-Token': this.token };
                                    $.ajax(this.varTableURL + "/" + this.varTableT_RESP_ORDEN, {
                                        headers: oHeaders,
                                        data: oVectorJSON,
                                        type: "POST",
                                        contentType: "application/json",
